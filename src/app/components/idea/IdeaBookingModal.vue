<template>
  <app-dialog
      ...
      :fullscreen = "isMobile"
  >
    <div class="booking-modal" v-if="idea && date && this.idea.relationships.author.attributes.whatsapp">
    ...
    <div v-else>
      <div class="booking-modal__title">{{idea.attributes.booking_info ? idea.attributes.booking_info : $t('idea.bookingInfoDefault')}}</div>
      <div class="booking-modal__title">{{idea.attributes.booking_contacts}}</div>
    </div>
  </app-dialog>
...
export default {
  ...
  computed: {
    whatsAppUrl(){
      ...
      let bookingPhone = this.idea.relationships.author.attributes.whatsapp ? this.idea.relationships.author.attributes.whatsapp : '79635150150'
